...
function Generate(){
    ...
    if(items.value == 0){
        data.innerHTML = "";
        alert("Input Value Can Not Be Zero");
    }
    else if (items.value > paragraphs.length){
        const randomIndex = Math.floor(Math.random() * paragraphs.length);
        data.innerHTML = `${paragraphs[randomIndex]}`;
    }else{
        const shuffledparagraphs = paragraphs;
        shuffle(paragraphs);

        const selectedparas = shuffledparagraphs.slice(0,items.value);
        const parasHTML = selectedparas.map(paragraphs => `<p>${paragraphs}</p>`).join("");
        data.innerHTML = parasHTML;
    }
